-- ============================================================
-- 109_ugc_template_v3.sql   GENERATED - do not hand-edit.
--   node scripts/build-legal-seed.mjs
--
-- Ports the live "Rawad altathir UGC" contract into legal.*: 16 typed
-- fields and 42 blocks as version 3, published, in workspace
...
begin
  ...
  select id into v_ver from legal.doc_template_version
   where template_id = v_tpl and version = 3;
  if v_ver is null then
    ...
    values (v_tpl, v_ws, 3, 'draft') returning id into v_ver;
    ...
    (v_ver, v_ws, 2, 'title', '{"text":"\" اتفاقية تسويق الكتروني\""}'::jsonb, false, null, null),
    (v_ver, v_ws, 3, 'p', '{"text":"الحمد لله وحده واما بعد:"}'::jsonb, false, null, null),
    (v_ver, v_ws, 4, 'p', '{"text":"أنه في تاريخ {{ date }} م. الموافق {{ day }} حررت هذه الاتفاقية (يشار اليها \"بالاتفاقية\") بين كل من:"}'::jsonb, false, null, null),
    ...
    (v_ver, v_ws, 8, 'h', '{"text":"أولًا: يعتبر التمهيد جزءًا لا يتجزأ من هذه الاتفاقية:"}'::jsonb, false, null, null),
    ...
    (v_ver, v_ws, 14, 'p', '{"text":"ان يسدد الطرف الاول لـ الطرف الثاني مبلغ الاعلان وقدره {{ Amount_full }} غير شاملة الضريبة، وذلك عن طريق التحويل المصرفي للحساب البنكي وستكون الحوالة خلال {{ duration }} يوم بعد انتهاء الحملة الاعلانية."}'::jsonb, false, null, null),
    ...
    (v_ver, v_ws, 16, 'p', '{"text":"سيتم التحويل بالمعلومات التالية:"}'::jsonb, false, null, null),
    ...
    (v_ver, v_ws, 23, 'li', '{"text":"- الشروط الباطلة: إن بطلان أي بند من بنود هذا \"العقد\" لا يؤثر على بقية بنود \"العقد\" التي تظل سارية ومنتجة لأثارها بين \"الطرفين\". الا اذا تبين ان احد المتعاقدين ما كان ليرضى بالعقد دون ذلك البند فله حق طلب ابطال العقد."}'::jsonb, true, 'terms', null),
    (v_ver, v_ws, 24, 'li', '{"text":"- يجب ارسال إحصاءات الإعلان بعد نزوله:"}'::jsonb, true, 'terms', null),
    (v_ver, v_ws, 25, 'li', '{"text":"- إحصاءات \" تيك توك\" , \" انستقرام بوست و رييل\" , لا تتجاوز 7 أيام من تاريخ نزول الإعلان."}'::jsonb, true, 'terms', null),
    (v_ver, v_ws, 26, 'li', '{"text":"- إحصاءات \" انستقرام و سناب تشات ستوري\" لا تتجاوز 24 ساعة من تاريخ نزول الإعلان."}'::jsonb, true, 'terms', null),
    (v_ver, v_ws, 27, 'h', '{"text":"خامساً: إنهاء العقد او طلب تغيير في موضوع العقد:"}'::jsonb, true, 'termination', 'خامساً: إنهاء العقد او طلب تغيير في موضوع العقد:'),
    (v_ver, v_ws, 28, 'p', '{"text":"في حال رغب \"الطرف الثاني\" في انهاء العقد و يوجد منتجات مرسلة من قبل \"الطرف الأول\" يلتزم \"الطرف الثاني\" في ارجاعها مع تحمل قيمة الارجاع ان وجدت."}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 29, 'p', '{"text":"في حال رغب \"الطرف الثاني\" في انهاء العقد و يوجد مبالغ تم دفعها \"للطرف الثاني\" من قبل \"الطرف الأول\" يلتزم \"الطرف الثاني\" في ارجاعها."}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 30, 'p', '{"text":"لا يمكن \" للطرف الثاني\" طلب تقليل عدد الخدمات المتفق عليها في \" موضوع العقد \". حيث ان الأسعار المعطاة مرتبطة ارتباط كلي بعدد الخدمات الكمي."}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 31, 'p', '{"text":"يتم ارجاع المبالغ عن طريق التحويل البنكي على حساب المؤسسة بالمعلومات التالية:"}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 32, 'kv', '{"label":"البنك","value":"الانماء"}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 33, 'kv', '{"label":"اسم الحساب","value":"شركة رواد التأثير"}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 34, 'kv', '{"label":"رقم الحساب","value":"68202707824000"}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 35, 'kv', '{"label":"رقم الايبان","value":"SA8505000068202707824000"}'::jsonb, true, 'termination', null),
    (v_ver, v_ws, 36, 'h', '{"text":"سادساً: الإخطار والمراسلات:"}'::jsonb, true, 'notices', 'سادساً: الإخطار والمراسلات:'),
    ...
    (v_ver, v_ws, 38, 'h', '{"text":"سابعاً: النزاعات:"}'::jsonb, true, 'disputes', 'سابعاً: النزاعات:'),
    (v_ver, v_ws, 39, 'p', '{"text":"في حالة نشوء نزاع بين الطرفين \"لا سمح الله\" يلجأ لمحاكم المملكة العربية السعودية للفصل في هذا النزاع."}'::jsonb, true, 'disputes', null),
    (v_ver, v_ws, 40, 'p', '{"text":"تم تحرير هذا العقد من نسختين، وتسليم كل طرف نسخة منه. وعليه جرى التوقيع:"}'::jsonb, false, null, null),
    (v_ver, v_ws, 41, 'p', '{"text":"والله الموفق."}'::jsonb, false, null, null),
    (v_ver, v_ws, 42, 'sig', '{"right":"الطرف الأول:  احمد قرنفلة","left":"الطرف الثاني:"}'::jsonb, false, null, null);
    ...
    raise notice 'version 3 already exists - blocks left alone';
  end if;
