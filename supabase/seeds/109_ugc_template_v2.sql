-- ============================================================
-- 109_ugc_template_v2.sql   GENERATED - do not hand-edit.
--   node scripts/build-legal-seed.mjs
--
-- Ports the live "Rawad altathir UGC" contract into legal.*: 16 typed
-- fields and 42 blocks as version 2, published, in workspace
...
begin
  ...
  select id into v_ver from legal.doc_template_version
   where template_id = v_tpl and version = 2;
  if v_ver is null then
    ...
    values (v_tpl, v_ws, 2, 'draft') returning id into v_ver;
    ...
    (v_ver, v_ws, 12, 'table', '{"columns":[{"key":"name_2","label":"المؤثر"},{"key":"platform_smart","label":"المنصة"},{"key":"channel_name","label":"حسابه في المنصة"},{"key":"ad_types","label":"نوع الإعلان"}],"row_source":"fields"}'::jsonb),
    ...
    raise notice 'version 2 already exists - blocks left alone';
  end if;
