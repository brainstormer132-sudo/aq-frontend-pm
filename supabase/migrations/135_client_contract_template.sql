...
begin
  ...
  -- 3. the version -------------------------------------------
  --
  -- The next version number, as a DRAFT. Never overwrites: if a version with
  -- these blocks is already there this does nothing, so re-running is safe and
  -- an existing draft somebody has been editing is not touched.
  if exists (select 1 from legal.doc_template_version v
             ...
             where v.template_id = v_tpl
               and b.content->>'text' = 'الحمد لله وحده واما بعد:') then
    raise notice 'seed: a version with this wording is already present - nothing to do';
    return;
  end if;
