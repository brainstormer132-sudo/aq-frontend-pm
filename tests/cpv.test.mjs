...
/* -- The template file, whichever DOCX built it ---------------------
 *
 * These used to assert "42 blocks" and an exact placeholder list, which
 * made the suite a description of one Word file: point the generator at
 * a different contract and it did not fail, it CRASHED - `.find(...).v`
 * on a placeholder that template did not have. What is asserted now is
 * what has to be true of any vendor agreement we can preview.
 */
const templateModule = require('../public/contracts/contract-template-ar.js');
const template = templateModule.template;
const templateValues = contractFieldValues(complete(), { today: '2026-09-08', strings: template.strings });
...
eq('it says which file it came from', template.label.length > 0, true);
eq('it is a whole document, not a fragment', template.blocks.length > 20, true);
eq('exactly one title', template.blocks.filter((b) => b.t === 'title').length, 1);
eq('at most one signature row', template.blocks.filter((b) => b.t === 'sig').length <= 1, true);
eq('the typed underscore rules are dropped, not printed',
  template.blocks.filter((b) => /_{6,}/.test(b.v || '')).length, 0);
eq('it carries the words the preview prints for a gap and for a pending field',
  [template.strings.missing.length > 0, template.strings.pending.length > 0, template.strings.riyal.length > 0],
  [true, true, true]);
...
// The one that matters: a placeholder the form cannot fill would print
// the not-entered wording forever, and nobody would know why.
eq('every placeholder in this template is one the form knows how to fill',
  templatePlaceholders().filter((key) => !Object.keys(templateValues).includes(key)), []);

/* -- Filling the document -------------------------------------------- */
eq('the date reads the way the document writes dates', templateValues.date.value, '08/09/2026');
eq('the contract number waits for generation', templateValues.id.pending, true);
eq('so does the hijri date', templateValues.day.pending, true);
eq('the amount carries the template currency word',
  templateValues.Amount_full.value, '12,000.00 ' + template.strings.riyal);
eq('slashDate leaves an unparseable date alone', slashDate('soon'), 'soon');

const doc = contractDocument(template, templateValues);
eq('the document keeps every block', doc.blocks.length, template.blocks.length);
eq('and carries the label through', doc.label, template.label);
...
eq('every placeholder became a field segment',
  fieldSegs(doc).length >= templatePlaceholders().length, true);
eq('nothing is missing on a complete booking',
  fieldSegs(doc).filter((s) => s.missing).map((s) => s.key), []);
eq('only the fields the backend stamps are pending',
  [...new Set(fieldSegs(doc).filter((s) => s.pending).map((s) => s.key))]
    .filter((key) => !['id', 'day', 'date'].includes(key)), []);
eq('and a pending field prints the template wording for it',
  fieldSegs(doc).filter((s) => s.pending).every((s) => s.v === template.strings.pending), true);
...
eq('a missing brand and bank show up in the body, by key',
  [...new Set(fieldSegs(gapDoc).filter((s) => s.missing).map((s) => s.key))].sort(),
  ['account_name', 'account_number', 'bank_name', 'brand_name', 'iban']);
eq('and they read as not-entered rather than blank',
  fieldSegs(gapDoc).filter((s) => s.missing).every((s) => s.v === template.strings.missing), true);

/* -- The services table, when the template has one ------------------- */
const tableBlock = doc.blocks.find((b) => b.t === 'table');
if (tableBlock) {
  eq('the header row is plain text',
    (tableBlock.rows[0] || []).every((c) => c.every((s) => s.t === 'text')), true);
  eq('the data row is filled from the booking',
